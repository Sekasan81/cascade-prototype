export const VERSION = 'station-0.1.0';
export const RACES = {
...
export const FIELDS = {
  credits:[80,0,1000,'Кредиты на старте','Старт и запасы'], bio:[8,0,100,'Биоматериал на старте','Старт и запасы'], water:[9,0,100,'Вода на старте','Старт и запасы'],
  energy:[8,0,100,'Энергия на старте','Старт и запасы'], reserve:[36,0,200,'Заряд батареи на старте','Старт и запасы'],
  bioCap:[16,4,100,'Емкость биоматериала','Старт и запасы'], waterCap:[18,3,100,'Емкость воды','Старт и запасы'], energyCap:[12,4,100,'Емкость энергии','Старт и запасы'], reserveCap:[72,1,200,'Емкость заряда батарей','Старт и запасы'],
  ...
  convertEnergy:[4,0,20,'Перестройка: энергия','Действия'], convertBio:[3,1,20,'Перестройка: био','Действия'], convertWater:[3,0,20,'Переход в воду: вода','Действия'], acceptWater:[1,0,10,'Прием аквари: вода','Действия'], upgradeEnergy:[4,0,20,'Улучшение: энергия','Действия'],
  batteryPrice:[18,1,100,'Цена батареи','Энергия и производство'], batterySize:[36,1,200,'Заряд одной батареи','Энергия и производство'], recharge:[2,1,20,'Передача заряда за такт','Энергия и производство'],
  period:[3,1,12,'Период производства','Энергия и производство'], bioYield:[2,1,20,'Выпуск био','Энергия и производство'], waterYield:[3,1,20,'Выпуск воды','Энергия и производство'], productionEnergy:[2,0,20,'Энергия на один выпуск','Энергия и производство'],
  generatorPrice:[60,1,500,'Цена генератора','Улучшения'], generatorBio:[6,0,30,'Генератор: био','Улучшения'], generatorXP:[24,0,200,'Генератор: порог XP','Улучшения'], generatorPeriod:[3,1,12,'Генератор: период','Улучшения'], generatorYield:[3,1,20,'Генератор: выпуск энергии','Улучшения'], fuelSize:[12,1,100,'Емкость топливного бака','Улучшения'], fuelPrice:[4,1,100,'Цена полного бака','Улучшения'],
  capacityPrice:[35,1,300,'Цена накопителя','Улучшения'], capacityBio:[4,0,30,'Накопитель: био','Улучшения'], extraCapacity:[6,1,50,'Добавочная емкость энергии','Улучшения'],
  efficientPrice:[45,1,300,'Цена экономной перестройки','Улучшения'], efficientBio:[4,0,30,'Улучшение перестройки: био','Улучшения'], efficientSaving:[1,1,10,'Экономия био при перестройке','Улучшения'],
  shiftTicks:[24,18,48,'Рабочих тактов в смене','Смены и прогресс'], sleepTicks:[6,1,12,'Тактов сна','Смены и прогресс'], upkeep:[12,0,100,'Содержание за смену','Смены и прогресс'], target:[16,1,22,'Отправок для победы','Смены и прогресс'], bXP:[12,0,200,'Направление B: XP','Смены и прогресс'], cXP:[36,0,300,'Направление C: XP','Смены и прогресс'],
  repTourist:[4,1,100,'Турист: репутация','Смены и прогресс'], repDiplomat:[10,1,150,'Дипломат: репутация','Смены и прогресс'], repTrader:[16,1,200,'Торговец: репутация','Смены и прогресс'],
};
for (let i=0;i<4;i++) {
  const name=CATEGORIES[i];
  for(const [key,values,min,max,label] of [ ['reward',[18,26,36,42],0,200,'кредиты'], ['xp',[3,4,6,7],0,30,'XP'], ['rep',[2,3,4,4],0,20,'репутация'], ['queue',[12,10,8,8],1,40,'срок в очереди'], ['room',[14,12,10,10],2,40,'срок в комнате'] ])
...
export const SCHEDULE=[['dry','dry','water'],['dry','water','gas'],['water','gas','dry','dry'],['gas','dry','water','dry'],['water','gas','dry','dry'],['gas','water','dry','dry']];
export const arrivalTicks=shift=>shift<=2?[1,8,15]:[1,6,11,16];
...
export function newGame(config={}) {
  ...
  const s={version:VERSION,cfg,shift:1,tick:0,world:0,credits:cfg.credits,bio:cfg.bio,water:cfg.water,energy:cfg.energy,reserve:cfg.reserve,fuel:0,xp:0,rep:{dry:0,water:0,gas:0},unlocked:{dry:0,water:0,gas:0},rooms:['dry','water'],upgrades:{generator:false,capacity:false,efficient:false},production:{bio:false,water:false},guests:[],stories:{neri:{helped:false,gift:false},ro:{helped:false,gift:false}},sent:0,lost:0,skipped:0,ended:false,ending:'',log:[],summaries:[],nextId:1};
  spawn(s,0,[]);return s;
}
...
function spawn(s,slot,events) {
  const race=SCHEDULE[s.shift-1][slot]; if(!race)return;
  if(s.guests.some(p=>p.shift===s.shift&&p.slot===slot))return;
  let category=s.shift===1?0:s.unlocked[race]; const available=routes(s);
  if(category===1&&available.length<2) category=0;
  const story=(s.shift===1&&slot===2)||(s.shift===4&&slot===2)?'neri':(s.shift===3&&slot===2)||(s.shift===6&&slot===3)?'ro':null;
  const names={dry:['Тим','Эно','Лис','Мио','Сан'],water:['Уна','Оли','Каи','Луми'],gas:['Фло','Иви','Зеф','Эхо']};
  const id=s.nextId++;
  const p={id,race,category,shift:s.shift,slot,story,visit:story?(s.shift<=3?1:2):0,name:story==='neri'?'Нери':story==='ro'?'Ро':names[race][(id-1)%names[race].length],status:'queue',room:null,route:null,allowed:category===0?null:category===1?available.slice(0,2):[available[(id-1)%available.length]],remaining:s.cfg['queue'+category],help:false,traded:false};
  s.guests.push(p);
  if(s.guests.filter(g=>g.status==='queue').length>3) lose(s,p,'Очередь переполнена',events);
  else events.push(`Прибыл ${p.name}: ${CATEGORIES[category].toLowerCase()}`);
}
...
export function inspect(s,cmd) {
  ...
  switch(cmd.type) {
    ...
    case 'accept':
      ...
      cost={energy:s.cfg.acceptEnergy,water:p.race==='water'?s.cfg.acceptWater:0};label=`Прием: ${p.name}`;break;
    case 'route':
    ...
    case 'trade':
      ...
      if(s.bio+4>s.cfg.bioCap)return deny('Нужно место для 4 био');
      cost={credits:8};label=`${p.name}: покупка 4 био`;break;
...
function lose(s,p,reason,events) {
  const penalty=p.status==='room'?2:1;
...
function finishShift(s,events) {
  ...
  s.skipped+=SCHEDULE[s.shift-1].length-appeared;
  ...
  s.summaries.push({shift:s.shift,ticks:s.tick,sent:guests.filter(p=>p.status==='sent').length,lost:guests.filter(p=>p.status==='lost').length,skipped:SCHEDULE[s.shift-1].length-appeared,...snapshot(s),shiftSent:guests.filter(p=>p.status==='sent').length,shiftLost:guests.filter(p=>p.status==='lost').length});
  ...
  } else {s.shift++;s.tick=0;spawn(s,0,events);}
}
export function act(state,cmd) {
  ...
  switch(cmd.type) {
    ...
    case 'accept':p.status='room';p.room=cmd.room;p.remaining=s.cfg['room'+p.category];newlyAccepted=p.id;break;
    ...
    case 'send':
      ...
      if(p.story&&p.visit===2&&s.stories[p.story].helped&&!s.stories[p.story].gift) {
        ...
        if(p.story==='neri') {const gift=Math.min(4,s.cfg.bioCap-s.bio);s.bio+=gift;events.push(`Подарок Нери: +${gift} био${gift<4?` (${4-gift} не поместилось)`:''}`);}
    ...
    case 'trade':p.traded=true;s.bio+=4;break;
  }
  ...
  else {const slot=arrivalTicks(s.shift).indexOf(s.tick+1);if(slot>=0)spawn(s,slot,events);}
